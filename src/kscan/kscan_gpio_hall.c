...
#define KSCAN_ADC_GET_BY_IDX(node_id, idx)                                     \
  ((struct kscan_adc){.spec = ADC_DT_SPEC_GET_BY_IDX(node_id, idx),            \
                      .index = idx})
...
struct measurement_stats {
  size_t index;
  bool clean;
  struct pair values[MESS_COUNT];
};

static int16_t current_value;

static struct measurement_stats mess = {};

static void meas(uint16_t value) {
  int64_t now = k_uptime_get();

  if (mess.clean && mess.index != 0) {
    for (size_t i = 0; i < MESS_COUNT; ++i) {
      mess.values[i].time = 0;
      mess.values[i].value = 0;
    }
    mess.clean = false;
    mess.index = 0;
    return;
  ...
  if (mess.index == 0) {
    mess.values[mess.index].time = 0;
  } else {
    mess.values[mess.index].time = now - mess.values[mess.index - 1].time;
  }

  mess.values[mess.index].value = value;

  mess.index++;
}

static void collect(int16_t value, int16_t perc) {
  ...
  if (perc == 100 && !enough) {
    LOG_DBG("============= SECTION START =============");
    // print it

    for (size_t i = 0; i < mess.index; ++i) {
      LOG_DBG("[%i : %i]", mess.values[i].time, mess.values[i].value);
    }
    LOG_DBG("============= SECTION END =============");
    enough = true;
  }
...
int kscan_adc_read(const struct kscan_adc *adc) {
  ...
  if (err != 0) {
    LOG_ERR("Failed to read %s, channel: %i", adc->spec.dev->name,
            adc->spec.channel_id);
    return err;
  }
...
void print_key_stats(struct kscan_key_state *state) {
  LOG_DBG("Rest value: %i, Pressed value: %i, Distance %i", state->idle_value,
          state->max_value, state->range);
}
...
static int kscan_hall_read(const struct device *dev) {
  ...
  for (int i = 0; i < data->inputs.len; i++) {
    ...
    if (state->max_value == 0) {
      state->max_value = state->min_value = state->last_value =
          current_value + 1;
      continue;
    }
    ...
    int value =
        (abs(state->last_value - state->idle_value) / (float)state->range) *
        100;

    input_report(dev, INPUT_EV_MSC, adc->index, value, 0, K_NO_WAIT);
...
static int kscan_hall_init_input_inst(struct kscan_adc *adc) {
  ...
  if (err != 0) {
    LOG_ERR("Unable to configure channel %u of device %s. Err: %i",
            spec->channel_id, spec->dev->name, err);
    return err;
  }
  ...
  LOG_DBG("%s: AIN%u setup returned %d", spec->dev->name, spec->channel_id,
          err);
  return 0;
}

static int kscan_hall_init_idle_value(const struct kscan_adc *adc,
                                      struct kscan_key_state *state,
                                      int steps) {
  int32_t sum = 1;
...
static void kscan_hall_work_handler(struct k_work *work) {
  ...
  struct kscan_hall_data *data =
      CONTAINER_OF(dwork, struct kscan_hall_data, work);
...
static int compare_ports2(const void *a, const void *b) {
  ...
  return (adc_a->spec.dev + adc_a->spec.channel_id) -
         (adc_b->spec.dev + adc_b->spec.channel_id);
}
...
static int kscan_hall_configure(const struct device *dev,
                                kscan_callback_t callback) {
  struct kscan_hall_data *data = dev->data;
...
#define KSCAN_HALL_INIT(n)                                                     \
  ...
  DEVICE_DT_INST_DEFINE(n, &kscan_hall_init, NULL, &kscan_hall_data_##n,       \
                        &kscan_hall_config_##n, POST_KERNEL,                   \
                        CONFIG_KSCAN_INIT_PRIORITY, &kscan_hall_api);
